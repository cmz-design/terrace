...
void consumer(CBlockQueue *bq,Graph & G)
{
    while (flag) 
    {
        pair_uint data;
        ...
        bq->pop(data);
        mt_con.unlock();
        G.add_edge(data.x,data.y);
    }
}

void producer(CBlockQueue *bq) 
{
    for (;pro_num< all_num;pro_num ++) 
    {
        mt_prod.lock();
        bq->push(edges[pro_num]);
        mt_prod.unlock();
    }
    if(pro_num==all_num) flag=0;
...
/*       
      ...
      CBlockQueue bqueue;
      all_num=updates_to_run;

      vector<thread> th_prods;
      for (int i = 0; i < num_prod; ++i) 
      {
          th_prods.emplace_back(producer, &bqueue);
      }

      vector<thread> th_cons;
      for (int i = 0; i < num_con; ++i)
      {
          th_cons.emplace_back(consumer, &bqueue, std::ref(graph));
      }
      gettimeofday(&start, &tzp);
      for (auto &t : th_prods) 
      {
